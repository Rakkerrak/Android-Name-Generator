...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        showName = (TextView) findViewById(R.id.normName);
        showExpName = (TextView) findViewById(R.id.experName);
...
//      Combines female/male name lists into one
        ...
        if ((femaleCheck.isChecked() & maleCheck.isChecked())){
            name = name + allFirsts[r.nextInt(allFirsts.length)] + " ";

        }
        else if (maleCheck.isChecked()){
            name = name + mFirsts[r.nextInt(mFirsts.length)] + " ";
        }
        else if (femaleCheck.isChecked()){
            name = name + fFirsts[r.nextInt(fFirsts.length)] + " ";
        }
        else{
            name = name + allFirsts[r.nextInt(allFirsts.length)] + " ";
        }

//      Adds last name to randomly generated first name
        name = name + lasts[r.nextInt(lasts.length)];

        if (showName != null) showName.setText(name);
    }

//    Can these functions be deleted?
    ...
    public void expGen(View view) {
//      Button: Generates two experimental names and adds them to the string.
        String name = genName(numGen(1,3));
        String name2 = genName(numGen(1,3));
        String fullName = toTitle(name) + " " + toTitle(name2);
        if (showExpName != null) showExpName.setText(fullName);
    }
